...
OneWire * DallasTemperature::getOneWire()
{
    return _wire;
}
...
bool DallasTemperature::requestTemperaturesByAddress(const uint8_t *
                                                     deviceAddress)
{
...
int16_t DallasTemperature::getTemp(const uint8_t * deviceAddress)
{
    ...
    if(isConnected(deviceAddress,
                   scratchPad)) return calculateTemperature(deviceAddress, scratchPad);
...
char DallasTemperature::getHighAlarmTemp(const uint8_t * deviceAddress)
{
    ...
    ScratchPad scratchPad;
    if(isConnected(deviceAddress,
                   scratchPad)) return (char)scratchPad[HIGH_ALARM_TEMP];
    return DEVICE_DISCONNECTED_C;
...
char DallasTemperature::getLowAlarmTemp(const uint8_t * deviceAddress)
{
    ...
    ScratchPad scratchPad;
    if(isConnected(deviceAddress,
                   scratchPad)) return (char)scratchPad[LOW_ALARM_TEMP];
    return DEVICE_DISCONNECTED_C;
...
void * DallasTemperature::operator new(unsigned int
                                       size) // Implicit NSS obj size
{
